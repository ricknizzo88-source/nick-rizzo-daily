...
export const DEFAULT_ABOUT_CONTENT = {
  ...
  videoUrls: ["https://www.youtube.com/watch?v=4LIt-vYJRuo"]
};
...
export function aboutVideoUrls(content) {
  if (Array.isArray(content?.videoUrls)) {
    return content.videoUrls.map((url) => String(url).trim()).filter(Boolean);
  }

  return String(content?.videoUrl ?? "").trim()
