...
export async function loadDirectoryPlaces({ admin = false } = {}) {
  ...
  const { data: videos, error } = await supabase
    .from("food_videos")
    .select(
      "id, title, dish_name, permalink, thumbnail_url, published_at, restaurant_id, restaurants(id, name, slug, city, state:region, category:notes)"
    )
  ...
  const { data: restaurants, error: restaurantError } = await supabase
    .from("restaurants")
    .select("id, name, slug, city, state:region, category:notes")
